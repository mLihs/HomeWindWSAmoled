...
#include "esp_log.h"
#include "rom/ets_sys.h"  // For ets_delay_us() - microsecond delay
...
{
  // FIXED: vTaskDelay expects ticks, not milliseconds!
  // pdMS_TO_TICKS converts ms to tick count based on configTICK_RATE_HZ
  vTaskDelay(pdMS_TO_TICKS(ms));
}

void qmi8658_delay_us(unsigned int us)
{
  // FIXED: vTaskDelay cannot do microsecond delays (minimum is 1 tick = typically 1-10ms)
  // Use ets_delay_us() for accurate microsecond delays (busy-wait, blocks CPU)
  // Note: For very short delays this is acceptable; for longer delays consider yielding
  ets_delay_us(us);
}
...
void qmi8658_config_pedometer(unsigned short odr)
{
	...
	unsigned char ped_sig_count = 1;//Ʋ1
...
// REMOVED: qmi8658c_example() - unused example function
// The IMU is initialized via lcd_start_imu_rotation_task() instead
// If standalone IMU testing is needed, uncomment below:
/*
